...
import type {Locale, LocaleDayPeriod, Localize} from "../../../locale/types.js";
import type {
    Day,
    Era,
...
export const formatters: { [token: string]: Formatter } = {
    ...
    z: function (date, token, _localize, formatterOptions) {
        ...
        switch (token) {
            ...
            case "zz":
            case "zzz":
                return (tzIntlTimeZoneName('short', date, locale)) ?? ("GMT" + formatTimezoneShort(date.getTimezoneOffset(), ":"));
            // Long
            case "zzzz":
            default:
                return (tzIntlTimeZoneName('long', date, locale)) ?? ("GMT" + formatTimezone(date.getTimezoneOffset(), ":"));
        }
    },
...
function tzIntlTimeZoneName(
    ...
    if (date.timeZone) {
        ...
        const dtf = new Intl.DateTimeFormat(givenLocale ? [givenLocale.code, 'en-US'] : undefined, {
